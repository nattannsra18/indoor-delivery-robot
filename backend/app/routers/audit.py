...
def list_audit(offset: int = Query(0, ge=0), limit: int = Query(30, ge=1, le=100), action: str | None = Query(None, max_length=80), db: Session = Depends(get_db, scope="function")):
    if action is not None and action not in ALLOWED_ACTION_FILTERS:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="Unsupported audit action filter")
