...
async def acknowledge(alert_id: str, alerts: AlertService = Depends(service), user: UserORM = Depends(require_admin)):
    ...
    publish_committed_notifications(alerts.db, alerts.pending_notification_ids)
    await broadcast("acknowledged", Alert.model_validate(alert))
    return alert


@router.post("/{alert_id}/resolve", response_model=Alert)
async def resolve(alert_id: str, alerts: AlertService = Depends(service), user: UserORM = Depends(require_admin)):
    alert = alerts.resolve(alert_id, user.id)
    publish_committed_notifications(alerts.db, alerts.pending_notification_ids)
