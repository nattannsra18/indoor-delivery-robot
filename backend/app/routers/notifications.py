...
def list_notifications(offset: int = Query(0, ge=0), limit: int = Query(30, ge=1, le=100), category: NotificationCategory | None = Query(None), unread_only: bool = Query(False), db: Session = Depends(get_db, scope="function"), user: UserORM = Depends(require_user)):
...
def read_many(payload: NotificationReadRequest, db: Session = Depends(get_db, scope="function"), user: UserORM = Depends(require_user)):
    return NotificationService(db).mark_many_read(user.id, payload.ids)
@router.post("/{notification_id}/read", response_model=Notification)
def read_notification(notification_id: str, db: Session = Depends(get_db, scope="function"), user: UserORM = Depends(require_user)):
...
def read_all(db: Session = Depends(get_db, scope="function"), user: UserORM = Depends(require_user)):
