...
type NotificationGroup = { key: string; taskId?: string; accountId?: string; entries: TimelineEntry[]; unreadCount: number; latestAt: string };
...
function buildGroups(items: Notification[]): NotificationGroup[] {
  ...
  for (const item of items) {
    const entityKey = item.entityId && ["task", "user"].includes(item.entityType ?? "") ? `${item.entityType}:${item.entityId}` : undefined;
    const key = entityKey ?? `event:${item.id}`;
    const group = groups.get(key) ?? { key, taskId: item.entityType === "task" ? item.entityId : undefined, accountId: item.entityType === "user" ? item.entityId : undefined, entries: [], unreadCount: 0, latestAt: item.createdAt };
...
export default function NotificationsPage() {
  ...
  return <section className="mx-auto max-w-6xl pb-10">
    ...
    {loading ? <div aria-live="polite" aria-busy="true" className="grid gap-3"><span className="sr-only">{t("loading")}</span>{[0,1,2].map((item) => <div key={item} className="h-32 animate-pulse rounded-2xl bg-slate-100" />)}</div> : groups.length === 0 ? <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><h2 className="font-bold text-slate-900">{filter === "all" ? t("noNotifications") : copy.caughtUp}</h2><p className="mt-1 text-sm text-slate-500">{copy.caughtUpHelp}</p></div> : <div className="space-y-4">{groups.map((group) => <article key={group.key} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/70 px-5 py-4"><div><div className="flex items-center gap-2"><h2 className="font-bold text-slate-950">{group.taskId ?? (group.accountId ? copy.accountRequests : copy.systemUpdates)}</h2>{group.unreadCount > 0 && <span className="rounded-full bg-blue-600 px-2 py-0.5 text-xs font-bold text-white">{group.unreadCount}</span>}</div><p className="mt-1 text-xs text-slate-400">{timeLabel(group.latestAt, locale)} · {copy.updateCount.replace("{count}", String(group.entries.reduce((sum, entry) => sum + entry.count, 0)))}</p></div><div className="flex items-center gap-2">{group.taskId && <Link href={`/tasks?task=${encodeURIComponent(group.taskId)}`} className="min-h-10 rounded-lg px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-50">{copy.viewDelivery} →</Link>}{group.accountId && <Link href="/users" className="min-h-10 rounded-lg px-3 py-2 text-sm font-bold text-amber-700 hover:bg-amber-50">{copy.reviewAccount} →</Link>}{group.unreadCount > 0 && <button type="button" disabled={marking} onClick={() => void markIds(group.entries.flatMap((entry) => entry.unreadIds))} className="min-h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600 disabled:opacity-50">{copy.markGroupRead}</button>}</div></header>
