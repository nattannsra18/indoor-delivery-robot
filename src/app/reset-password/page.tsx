...
import { getPasswordPolicy, resetPassword } from "@/lib/api";
...
export default function ResetPasswordPage(){
  ...
  useEffect(()=>{setToken(new URLSearchParams(window.location.search).get("token")??"");getPasswordPolicy().then(setPasswordPolicy).catch(()=>{});},[]);
  async function submit(event:FormEvent){event.preventDefault();setError("");if(!token){setError(copy.missingResetToken);return;}if(password!==confirm){setError(copy.passwordsMismatch);return;}const policyError=passwordPolicyError(password,passwordPolicy,{length:copy.passwordLength,letter:copy.passwordLetter,number:copy.passwordNumber});if(policyError){setError(policyError);return;}setBusy(true);try{await resetPassword(token,password);setComplete(true);}catch{setError(copy.invalidReset);}finally{setBusy(false);}}
  return <AuthShell><div className="mx-auto max-w-md">
    ...
    <p className="mt-2 text-sm leading-6 text-slate-500">{complete?copy.resetCompleteDetail:copy.resetSubtitle.replace("{count}",String(passwordPolicy.minimumLength))}</p>
    {complete?<Link href="/login" className="mt-8 inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-blue-600 px-4 font-semibold text-white">{copy.backToSignIn}</Link>:<form onSubmit={submit} className="mt-7 grid gap-5">
      <PasswordField label={copy.newPassword} value={password} onChange={setPassword} autoComplete="new-password" minLength={passwordPolicy.minimumLength}/>
