...
function MappingWorkspace({ robotId, session, busy, error, copy, locale, onBusy, onError, onSession, onSaved, onOpenLibrary }: { robotId: string; session?: MappingSession; busy: boolean; error: string; copy: (typeof webMappingText)[keyof typeof webMappingText]; locale: keyof typeof webMappingText; onBusy: (value: boolean) => void; onError: (value: string) => void; onSession: (value: MappingSession) => void; onSaved: () => void; onOpenLibrary: () => void }) {
  ...
  useEffect(() => {
    ...
    if (baselineSessionId.current !== session.sessionId) {
      baselineSessionId.current = session.sessionId;
    }
    // The session revision is generated by the Robot Agent, while the map
    // revision is generated by FastAPI.  They restart independently and must
    // never be compared.  A locally-started session already captures the
    // current FastAPI revision in the start button handler.  After a page
    // reload, use the first FastAPI map as the baseline and reveal the next
    // SLAM update from that same revision domain.
    setMappingStartRevision((current) => current ?? occupancyMap?.revision);
  }, [occupancyMap?.revision, phase, session?.sessionId]);
  useEffect(() => {
  ...
  return <div className="mt-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(340px,0.7fr)] 2xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.58fr)_minmax(340px,0.62fr)]">
      ...
      <aside className="grid content-start gap-4 2xl:contents">
        ...
        {phase === "IDLE" && <><p className="mt-5 rounded-2xl bg-blue-50 p-4 text-sm leading-6 text-blue-900">{copy.startHelp}</p><button type="button" disabled={busy} onClick={() => { setMappingStartRevision(occupancyMap?.revision ?? 0); void run(() => startMapping(robotId || undefined)); }} className="mt-4 min-h-12 w-full rounded-xl bg-blue-600 font-bold text-white disabled:opacity-50">{copy.start}</button></>}
