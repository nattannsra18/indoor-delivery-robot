  ...
  return (
    <div className="relative mb-5 flex justify-end">
      <button type="button" onClick={() => setOpen(!open)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold shadow-sm">
        🔔 {copy.alerts} {count > 0 && <span className="ml-1 rounded-full bg-red-600 px-2 py-0.5 text-xs text-white">{count}</span>}
      </button>
      {open && <section className="absolute right-0 top-12 z-50 max-h-[70vh] w-[min(92vw,30rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
        <h2 className="font-semibold">{t("activeAlerts")}</h2>
        {error && <p className="mt-2 text-sm text-red-700">{error}</p>}
        <div className="mt-3 space-y-3">{alerts.length === 0 ? <p className="text-sm text-slate-500">{t("noActiveAlerts")}</p> : alerts.map((alert) =>
          <article key={alert.id} className={`rounded-xl border p-3 ${alert.severity === "CRITICAL" ? "border-red-300 bg-red-50" : "border-amber-200 bg-amber-50"}`}>
            ...
            <p className="mt-2 text-xs text-slate-500">{alert.robot_id ?? copy.system} · {new Date(alert.latest_occurrence_at).toLocaleString(locale)} · ×{alert.occurrence_count}</p>
            <div className="mt-2 flex gap-2">{!alert.acknowledged && <button onClick={() => void act(alert, "ack")} className="rounded-lg bg-slate-900 px-2 py-1 text-xs text-white">{t("acknowledge")}</button>}<button onClick={() => void act(alert, "resolve")} className="rounded-lg border border-slate-300 px-2 py-1 text-xs">{t("resolve")}</button></div>
          </article>)}
        </div>
      </section>}
    </div>
