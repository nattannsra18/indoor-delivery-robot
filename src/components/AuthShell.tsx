...
export function PasswordField({label,value,onChange,autoComplete="current-password",placeholder,minLength,describedBy,invalid}:{label:string;value:string;onChange:(value:string)=>void;autoComplete?:string;placeholder?:string;minLength?:number;describedBy?:string;invalid?:boolean}) {
  const {locale}=useLocale();
  const copy=authText[locale];
  ...
  return <label className="grid gap-2 text-sm font-semibold text-slate-800">
    ...
    <span className="relative">
      ...
      <input type={visible?"text":"password"} autoComplete={autoComplete} value={value} onChange={(event)=>onChange(event.target.value)} placeholder={placeholder} minLength={minLength} required aria-invalid={invalid || undefined} aria-describedby={describedBy} className={`min-h-14 w-full rounded-xl border bg-white px-12 pr-14 font-normal outline-none transition placeholder:text-slate-400 focus:ring-4 ${invalid ? "border-red-400 focus:border-red-500 focus:ring-red-100" : "border-slate-300 focus:border-blue-500 focus:ring-blue-100"}`} />
