...
export default function RobotMapMock() {
  ...
  return (
    <div className="relative min-h-[310px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
      ...
      {activeTask && <div className="absolute left-[30%] top-[40%] h-2 w-[38%] rotate-6 rounded-full bg-blue-400/70" />}

      <MapPoint label="A" className="left-[27%] top-[37%]" />
      <MapPoint label="B" className="left-[40%] top-[65%]" />
      <MapPoint label="C" className="left-[69%] top-[45%]" />
      <MapPoint label="D" className="left-[74%] top-[68%]" />

      <div className="absolute left-[51%] top-[39%] -translate-x-1/2 -translate-y-1/2">
        <div className="grid h-12 w-12 place-items-center rounded-full border-4 border-white bg-blue-600 text-lg font-bold text-white shadow-lg">
          R
        </div>
        <div className="mt-2 rounded bg-slate-950 px-2 py-1 text-center text-[10px] font-semibold text-white shadow">
          {robot.name}
        </div>
      </div>

      <div className="absolute bottom-3 left-3 max-w-[85%] rounded-lg bg-white/90 px-3 py-2 text-xs text-slate-500 shadow-sm backdrop-blur">
        {activeTask
          ? `${activeTask.id}: ${stationName(activeTask.pickupStationId)} → ${stationName(activeTask.destinationStationId)}`
          : `Robot IDLE at x=${robot.x.toFixed(1)}, y=${robot.y.toFixed(1)}`}
      </div>
    </div>
  );
}
...
function MapPoint({ label, className }: { label: string; className: string }) {
  return (
    <div className={`absolute ${className}`}>
      <div className="grid h-8 w-8 place-items-center rounded-full border-4 border-white bg-emerald-500 text-xs font-bold text-white shadow">
        {label}
      </div>
    </div>
  );
