...
test("notification UI covers localized loading, empty, retry and bulk read controls", () => { assert.match(page,/t\("loading"\)/);assert.match(page,/t\("noNotifications"\)/);assert.match(page,/markNotificationsRead/);assert.match(page,/markAllNotificationsRead/);assert.match(page,/idr:notification/); });
